...
/**
 * Customized version of LoadOnDemandDescriptions, 
 * the original version uses ECoreUtil2 (From xtext) in method getResourceDescription
 * to resolve the resource from a URI. This works out to invoke the CDOResourceFactory, 
 * which will create an "un-attached" resource. This implementation, 
 * use a data provider instead. 
 * 
 * 
 * @author Sebastian Zarnekow - Initial contribution and API
 * @author Christophe Bouhier christophe.bouhier@netxforge.com
 */
public class CDOLoadOnDemandResourceDescriptions extends AbstractCompoundSelectable implements IResourceDescriptions {
	...
	/**
	 * Get the ResourceServiceProvider manager, and 
	 * get the resource description from it. 
	 * 
	 */
	public IResourceDescription getResourceDescription(URI uri) {
		...
		if (result == null) {
			...
			// Customized, version which get the original CDO resource from the CDO URI through
			// a session, the original EcoreUtil2, tries also to resolve the URI as an absolute URI, to the 
			// provided context resource, which doesn't apply for a CDO resource. 
			
//			Resource resource = EcoreUtil2.getResource(context, uri.toString());
			assert dataService != null : new java.lang.IllegalStateException("Data Service should be initialized");
			Resource resource = dataService.getProvider().getResource(LibraryPackage.Literals.LIBRARY);
