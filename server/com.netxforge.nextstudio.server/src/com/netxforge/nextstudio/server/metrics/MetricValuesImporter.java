...
import com.netxforge.nextstudio.server.ServerUtils;
import com.netxforge.nextstudio.server.metrics.NetworkElementLocator.IdentifierValue;
...
public class MetricValuesImporter {
	...
	private MappingStatistic createMappingStatistics(long startTime,
		...
		final DateTimeRange range = GenericsFactory.eINSTANCE
				.createDateTimeRange();
		range.setBegin(ServerUtils.getInstance().getXMLGregorianCalendarDate(
				new Date(startTime)));
		range.setEnd(ServerUtils.getInstance().getXMLGregorianCalendarDate(
				new Date(endTime)));
	...
	private void addMetricValue(MappingXLSColumn xlsColumn, Date timeStamp,
		...
		value.setTimeStamp(ServerUtils.getInstance()
				.getXMLGregorianCalendarDate(timeStamp));
