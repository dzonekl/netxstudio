...
public class CreateTestData extends AbstractDataProviderTest {
	...
	protected void setUp() throws Exception {
		...
		dataProvider.setCreateResourceInSeparateTransaction(false);
	...
	private void saveData() throws IOException {
		saveResourcesForEPackage(GenericsPackage.eINSTANCE);
		saveResourcesForEPackage(GeoPackage.eINSTANCE);
		saveResourcesForEPackage(LibraryPackage.eINSTANCE);
		saveResourcesForEPackage(MetricsPackage.eINSTANCE);
		saveResourcesForEPackage(OperatorsPackage.eINSTANCE);
		saveResourcesForEPackage(ProtocolsPackage.eINSTANCE);
		saveResourcesForEPackage(SchedulingPackage.eINSTANCE);
		saveResourcesForEPackage(ServicesPackage.eINSTANCE);
	}

	private void saveResourcesForEPackage(
			org.eclipse.emf.ecore.EPackage ePackage) throws IOException {
		for (final EClassifier eClassifier : ePackage.getEClassifiers()) {
			if (eClassifier instanceof EClass) {
				final EClass eClass = (EClass) eClassifier;
				dataProvider.getResource(eClass).save(Collections.emptyMap());
			}
		}
	}
	...
	private void createRFSService() {
		...
		dataProvider.getResource(SchedulingPackage.Literals.JOB).getContents().add(job);
	}
	...
	private MetricSource createMetricSource()
	...
	private java.util.List<Function> createFunctions(String id, int level) {
		...
		for (int i = 0; i < HIERARCHY_BREADTH; i++) {
			final Function function = LibraryFactory.eINSTANCE
					.createFunction();
			if (level == 0 && i ==0) {
				function.setName(id);
			} else {
			...
			if (level <= HIERARCHY_DEPTH) {
				function.getFunctions().addAll(
						createFunctions(id, level + 1));
			}
		}
		return functions;
	}
