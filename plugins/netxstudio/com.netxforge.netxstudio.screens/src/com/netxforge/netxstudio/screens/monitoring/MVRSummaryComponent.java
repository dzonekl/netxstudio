...
import com.netxforge.netxstudio.common.model.MVRSummary;
import com.netxforge.netxstudio.metrics.MetricValueRange;

public class MVRSummaryComponent extends AbstractSummaryComponent {

	private FormText mvrText;
	...
	protected void buildSummary() {
		...
		Label summaryLabel = formToolkit.createLabel(gridContent, "MVR: ",
				SWT.NONE);
		...
		mvrText = formToolkit.createFormText(gridContent, false);
		GridData gridData = new GridData(SWT.FILL, SWT.TOP, false, false, 1, 1);
		mvrText.setLayoutData(gridData);
		...
		Label rangesLabel = formToolkit.createLabel(gridContent, "Values: ",
				SWT.NONE);
	...
	public void fillSummary(IMonitoringSummary nonCastedSummary) {
		if (nonCastedSummary instanceof MVRSummary) {

			MVRSummary summary = (MVRSummary) nonCastedSummary;

			MetricValueRange target = summary.getTarget();

			mvrText.setText("<form><p><b>" + target.getIntervalHint() + "/"
					+ "</b></p></form>", true, false);
			...
			markersText.setText(
					...
					false);
			rangesText.setText(
					new Integer(target.getMetricValues().size()).toString(),
					false, false);
