 ...
 * 
 * To run these tests the CDO server has to be started on tcp port 2036. This
 * can be accomplished by using the CDOHibernateServer.launch configuration
 * which makes use of the cdo-server.xml config file in the config directory.
 * <p/>
 * The connection logic resides in the {@link BaseTest} parent class.
 * 
 * @author Martin Taal
 */
public class ResourceSetTest extends AbstractInjectedTestJUnit4 {
	...
	public void testCreatePersist() throws Exception {
		...
		final String name = "name " + System.currentTimeMillis(); //$NON-NLS-1$
		{
			...
			// Create an EMF adapter factory which constructed from the
			// registered adapter factoy descriptors.
			// as by extension: <extension
			...
			final AdapterFactoryEditingDomain domain = new AdapterFactoryEditingDomain(
					emfEditAdapterFactory, commandStack);

			final CDOTransaction transaction = session.openTransaction(domain
					.getResourceSet());
			...
			// Create an object and add it to the resource, using the edit
			// framework.
			...
			final Command c = new AddCommand(domain, resource.getContents(),
					country);
