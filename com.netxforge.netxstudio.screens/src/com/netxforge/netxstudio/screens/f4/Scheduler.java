...
public class Scheduler extends Composite {
	...
	public Scheduler(Composite parent, int style) {
		...
		txtFilterText = toolkit.createText(frmScheduledJobs.getBody(), "New Text", SWT.H_SCROLL | SWT.SEARCH | SWT.CANCEL);
		...
		TableViewer tableViewer = new TableViewer(frmScheduledJobs.getBody(), SWT.BORDER | SWT.FULL_SELECTION);
		...
		tableItem.setText(new String[] {"SGSN Attached", "Datacollection", "Yes"});
		
		Button btnAddButton = toolkit.createButton(frmScheduledJobs.getBody(), "+", SWT.NONE);
		...
		Button btnRemoveButton = toolkit.createButton(frmScheduledJobs.getBody(), "-", SWT.NONE);
		GridData gd_btnRemoveButton = new GridData(SWT.RIGHT, SWT.CENTER, false, false, 1, 1);
