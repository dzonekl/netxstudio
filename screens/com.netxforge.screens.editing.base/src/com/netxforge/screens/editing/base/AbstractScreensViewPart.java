...
//		this.getEditingService().disposeData();
			...
			getViewSite().getShell().getDisplay().asyncExec(new Runnable() {
				public void run() {
					...
					if (BaseEditingActivator.DEBUG) {
						BaseEditingActivator.TRACE.trace(null,
	...
	private void setStatusLineManager(ISelection selection) {
		if (selection instanceof IStructuredSelection) {
			...
			List<Object> newArrayList = Lists
					.newArrayList(((IStructuredSelection) selection).iterator());
			setStatusLineManager(newArrayList);
		}
