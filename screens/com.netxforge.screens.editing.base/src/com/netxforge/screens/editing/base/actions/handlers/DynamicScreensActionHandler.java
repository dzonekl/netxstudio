...
public class DynamicScreensActionHandler extends AbstractActionHandler {
	...
	private void addActions(IMenuManager menuManager) {
		...
		for (IAction action : actions) {
			if (BaseEditingActivator.DEBUG) {
				BaseEditingActivator.TRACE.trace("/debug",
	...
	public void handleSelection(IStructuredSelection ss) {
		...
		if (BaseEditingActivator.DEBUG && !ss.isEmpty()) {
			BaseEditingActivator.TRACE.trace(null, "update selection: " + ss);
		}

		for (IAction action : actions) {

			if (action instanceof com.netxforge.screens.editing.base.actions.BaseSelectionListenerAction) {
				com.netxforge.screens.editing.base.actions.BaseSelectionListenerAction bsla = (com.netxforge.screens.editing.base.actions.BaseSelectionListenerAction) action;
